# escape_game_full_4minigames.py
...
pygame.display.set_caption("Escape Game — 4 Minigames")
...
task_stations = [
    ...
    {"rect": pygame.Rect(1400,1200,96,96), "name":"Math",  "completed": False},
    {"rect": pygame.Rect(900,1600,96,96), "name":"Animal","completed": False},
]
...
class MathGame:
    ...
    def handle_event(self, event):
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_BACKSPACE:
                ...
                if self.user == self.answer:
                    self.completed = True
                    self.popup_open = False
            else:
                name = pygame.key.name(event.key)
                if name.isdigit() and len(self.user) < 3:
                    self.user += name
    ...
    def draw(self, surf):
        ...
        draw_rounded_rect(surf, rect.inflate(-10,-10), (24,28,34), radius=12)
        cx, cy = rect.center
        title = BIG.render("Solve:", True, (230,230,230))
        surf.blit(title, (cx - title.get_width()//2, cy - 80))
        qtxt = FONT.render(self.question, True, (240,240,240))
        surf.blit(qtxt, (cx - qtxt.get_width()//2, cy - 20))
        input_txt = FONT.render(f"Answer: {self.user}", True, (200,200,200))
        surf.blit(input_txt, (cx - input_txt.get_width()//2, cy + 30))

# ---------------- MINIGAME 4: Animal Guess ----------------
ANIMALS = ["cat","dog","cow","lion","tiger","bird","rabbit","horse"]
class AnimalGame:
    def __init__(self):
        self.target = random.choice(ANIMALS)
        self.user = ""
        self.popup_alpha = 0
        self.popup_open = True
        self.completed = False
    ...
    def handle_event(self, event):
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_BACKSPACE:
                ...
                if self.user.lower() == self.target:
                    self.completed = True
                    self.popup_open = False
            else:
                name = pygame.key.name(event.key)
                if name.isalpha() and len(self.user) < 8:
                    self.user += name
    ...
    def draw(self, surf):
        if self.popup_alpha == 0: return
        popup = pygame.Surface((560,300), pygame.SRCALPHA)
        popup.fill((12,14,20,self.popup_alpha))
        rect = popup.get_rect(center=(WIDTH//2, HEIGHT//2))
        surf.blit(popup, rect.topleft)
        draw_rounded_rect(surf, rect.inflate(-10,-10), (24,28,34), radius=12)
        cx, cy = rect.center
        title = FONT.render("Guess the animal (type name and Enter)", True, (230,230,230))
        surf.blit(title, (cx - title.get_width()//2, cy - 90))
        # give a hint (length)
        hint = FONT.render(f"Hint: {len(self.target)} letters", True, (200,200,200))
        surf.blit(hint, (cx - hint.get_width()//2, cy - 30))
        input_txt = FONT.render(f"Your guess: {self.user}", True, (200,200,200))
        surf.blit(input_txt, (cx - input_txt.get_width()//2, cy + 20))
...
active_game = None       # one of wires_game / number_game / math_game / animal_game
debug_draw = False
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if state == "menu":
            ...
            if active_game is not None:
                # wires uses mouse + space; others use keyboard
                if isinstance(active_game, WiresMinigame):
                    ...
                    # Number/Math/Animal classes implement handle_event
                    if event.type == pygame.KEYDOWN:
                        active_game.handle_event(event)
                        if event.type == pygame.KEYDOWN and event.key == pygame.K_q:
                            # cancel
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    if found:
                        ...
                        if idx == 0:
                            ...
                            math_game = MathGame(); active_game = math_game
                        elif idx == 3:
                            animal_game = AnimalGame(); active_game = animal_game
                    # pick up key
            ...
            if event.type == pygame.KEYDOWN:
                ...
                wires_game = WiresMinigame(); number_game = None; math_game = None; animal_game = None
    ...
    if isinstance(active_game, WiresMinigame):
        ...
        active_game.update_popup_anim()
    elif isinstance(active_game, MathGame):
        active_game.update_popup_anim()
    elif isinstance(active_game, AnimalGame):
        active_game.update_popup_anim()
    ...
    if state == "menu":
        ...
        title = BIG.render("Escape Game — 4 Minigames", True, (240,240,240)); screen.blit(title, (WIDTH//2 - title.get_width()//2, 120))
